      ...
      <div className="flex w-full max-w-4xl overflow-hidden rounded-lg border border-border/50 bg-card shadow-lg">
        ...
        <div className="relative hidden w-5/12 lg:block">
          ...
          <div className="absolute bottom-8 left-8 right-8">
            <h2 className="font-serif text-2xl text-cream">Join the Experience</h2>
            <p className="mt-2 font-sans text-sm text-cream/70">
              Create an account to unlock loyalty rewards, save favorite rooms, and manage your bookings.
            </p>
          </div>
        </div>
          ...
          <form onSubmit={handleSubmit} className="mt-8 flex flex-col gap-5">
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <div className="flex flex-col gap-1.5">
                ...
                <Input id="firstName" required value={form.firstName} onChange={(e) => update("firstName", e.target.value)} placeholder="John" />
              </div>
              <div className="flex flex-col gap-1.5">
                <Label htmlFor="lastName" className="font-sans text-xs uppercase tracking-wider text-muted-foreground">
                  Last Name
                </Label>
                <Input id="lastName" required value={form.lastName} onChange={(e) => update("lastName", e.target.value)} placeholder="Doe" />
              </div>
            </div>

            <div className="flex flex-col gap-1.5">
              <Label htmlFor="regEmail" className="font-sans text-xs uppercase tracking-wider text-muted-foreground">
                Email Address
              </Label>
              <Input id="regEmail" type="email" required value={form.email} onChange={(e) => update("email", e.target.value)} placeholder="your@email.com" />
            </div>

            <div className="flex flex-col gap-1.5">
              <Label htmlFor="regPassword" className="font-sans text-xs uppercase tracking-wider text-muted-foreground">
                Password
              </Label>
            ...
            <div className="flex flex-col gap-1.5">
              <Label htmlFor="confirmPassword" className="font-sans text-xs uppercase tracking-wider text-muted-foreground">
                Confirm Password
              </Label>
              <Input
                id="confirmPassword"
            ...
            <p className="text-center font-sans text-sm text-muted-foreground">
              ...
              <Link href="/account/login" className="font-medium text-gold hover:underline">
                Sign in
              </Link>
            </p>
          </form>
